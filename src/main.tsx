...
function Guidance() {
  ...
  const card = (kind: "icp" | "voice_guide" | "visual_guide", title: string, description: string) => {
    ...
    const latest = docs[0];
    return <article className="guidance-card"><span className="guidance-icon"><FiBookOpen /></span><h2>{title}</h2><p>{description}</p>{latest && <p className="last-updated">Last Updated {new Date(latest.created_at).toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" })}</p>}<label className="button primary wide"><FiUploadCloud /> {uploading === kind ? "Uploading…" : `Upload ${title}`}<input hidden type="file" accept={kind === "voice_guide" ? ".md,.txt,text/markdown,text/plain,application/text" : ".md,.pdf,.txt,.doc,.docx,text/markdown,application/text,application/pdf,text/plain,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document"} disabled={Boolean(uploading)} onChange={(e) => { void upload(kind, e.target.files?.[0]); e.currentTarget.value = ""; }} /></label><small>{kind === "voice_guide" ? "Markdown or TXT · editable below" : "Markdown, PDF, DOCX, DOC, or TXT · 10 MB max"} · private to your workspace</small>{kind === "voice_guide" && <><Field label="Editable GSD Voice"><textarea className="voice-editor" value={voiceText} onChange={(e) => setVoiceText(e.target.value)} placeholder="Upload a .md file or write the GSD Voice here…" /></Field><button className="button wide" onClick={() => void saveVoice()} disabled={savingVoice}>{savingVoice ? "Saving…" : "Save GSD Voice"}</button></>}{docs.length > 0 ? <div className="document-list">{docs.map((doc) => <div key={doc.id}><FiFileText /> <span>{doc.file_name}</span><FiCheck /></div>)}</div> : <div className="document-empty">No file uploaded yet.</div>}</article>;
  };
