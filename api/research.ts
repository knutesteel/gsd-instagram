...
type Candidate = {
  title: string; url: string; publisher?: string; category: string; rank: number;
  summary: string; post_type: "single_image" | "carousel" | "multi_pane_cartoon" | "reel";
  panel_count?: number; image_summary?: Record<string, string>; detailed_prompt: string;
...
export default async function handler(req: any, res: any) {
  ...
  const voice = docs.find((doc: any) => doc.kind === "voice_guide")?.text_content ?? "Use a warm, direct, non-judgmental GSD voice. Hank and the squirrel reveal an honest execution insight.";
  const icp = docs.find((doc: any) => doc.kind === "icp")?.text_content ?? "Knowledge workers who want practical, compassionate help protecting attention and following through.";
...
Follow this policy: original accessible sources only; no politics, celebrity gossip, routine sports, paywalls, aggregators, or sensational misinformation. Prioritize neuroscience/behavior, surprising animals, science/space, archaeology, offbeat human stories, attention technology, and immediately useful productivity. Use web search. Return ${mode === "manual" ? "one" : "three"} high-fit stories. Score rank 61-100 based on engagement for the GSD ICP. GSD ICP: ${icp}\nGSD VOICE: ${voice}\nReturn ONLY a JSON array. Each object must have title,url,publisher,category,rank,summary (25 words max),post_type, panel_count, image_summary (object with location,time_of_day,hank_expression,squirrel_expression), detailed_prompt (include Hank and squirrel dialogue, voice, continuity), caption,hashtags (array).`;
  ...
  const output = ai.output_text ?? ai.output?.flatMap((item: any) => item.content ?? []).find((part: any) => part.type === "output_text")?.text;
