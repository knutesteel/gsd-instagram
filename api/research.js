...
export default async function handler(req, res) {
  ...
  const image = docs.find((doc) => doc.kind === "visual_guide")?.text_content ?? "Hank is an adult raccoon and the squirrel is a squirrel. Never portray Hank as human.";
  const recency = String(timeframe) === "anytime" ? "with no publication-date limit" : `from the past ${Number(timeframe) || 48} hours`;
  const query = mode === "manual" ? `Analyze this exact direct article URL: ${manualUrl}. The returned url must exactly match it.` : `Find accessible stories ${recency}. Search focus: ${searchText || topics.join(", ")}.`;
...
GSD ICP PROMPT:\n${icp}\n\nGSD VOICE PROMPT:\n${voice}\n\nGSD IMAGE PROMPT:\n${image}\n\nUse all three saved prompts internally. In particular, Hank is the raccoon defined in the Image Prompt, never a human. Do not repeat any global voice, image, style, character, clothing, scale, palette, or generic continuity rule in the response.\n\nThe image_summary.content field is an ARTICLE-SPECIFIC panel brief only. Start directly with “Panel 1” — never begin with “Create a carousel,” “Use GSD voice,” a style description, or a generic continuity instruction. Separate every panel with one blank line and use this exact readable structure:\n\nPanel [number] — [short story beat]\nAction: [what Hank and the squirrel are each doing; mention only article-specific props or developments]\nHank: “exact line”\nthe squirrel: “exact line”\n\nEvery panel must include both Hank and the squirrel, and both must speak at least once. Do not use narration, text overlays, captions, or a panel with only one character in place of their conversation. Create a single causal mini-story across the panels: panel 1 introduces the article, middle panels explain/test its finding, and the final panel lands one humane takeaway. Keep the article-specific setting from image_summary.setting stable and carry forward relevant story props or actions between panels unless the panel explicitly changes them.\n\nDo not include voice-guide, style-guide, visual-guide, clothing, scale, color, palette, composition, image-generation instructions, or a source URL in content. Those rules come from saved prompts later. Do not return detailed_prompt.`;
