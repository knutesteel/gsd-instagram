...
GSD ICP PROMPT:\n${icp}\n\nGSD VOICE PROMPT:\n${voice}\n\nGSD IMAGE PROMPT:\n${image}\n\nUse all three saved prompts internally. In particular, Hank is the raccoon defined in the Image Prompt, never a human. Do not repeat any global voice, image, style, character, clothing, scale, palette, or continuity rule in the response.\n\nThe image_summary.content field is an ARTICLE-SPECIFIC panel brief only. Start directly with “Panel 1” — never begin with “Create a carousel,” “Use GSD voice,” a style description, or a continuity instruction. Separate every panel with one blank line. For each panel, include only the article-specific scene/action, Hank’s exact dialogue, and the squirrel’s exact dialogue. Every panel must show both characters discussing or reacting to the article. Do not include voice-guide, style-guide, visual-guide, clothing, scale, color, palette, composition, continuity, image-generation instructions, or a source URL in content. Those rules come from saved prompts later. Do not return detailed_prompt.`;
