...
export default async function handler(req, res) {
  ...
  const query = `${mode === "manual" ? `Analyze this exact direct article URL: ${manualUrl}. The returned url must exactly match it.` : mode === "overview" ? `Create one original Hank-and-the-squirrel suggested post from this text overview. Do not search for or cite a news article. Use this exact internal URL: ${overviewUrl}. Title the result with a short, descriptive title derived from the overview. Overview: ${overview}` : `Find accessible stories published within the last 90 days. Search focus: ${searchText || topics.join(", ")}. Return up to three qualifying stories from that 90-day window. Only if fewer than three qualifying stories exist in that window, widen the search to articles published within the last year and use those older results only to fill the remaining slots. Never search beyond the last year.`} Caption requirement: write a short GSD-voice comedic take, not a summary or recap. Start with a sharp, relatable observation about the absurdity, tension, or human truth behind the post. Use dry wit, self-awareness, and the Hank-versus-the-squirrel dynamic where it fits, while staying warm rather than mean. Do not restate the headline, list article facts, explain every panel, or use generic productivity advice. Format the caption exactly as three sections separated by one blank line: (1) the humorous article-specific take, (2) the exact source URL, and (3) a concise, article-specific question that invites a comment. For an overview without an article, omit the source URL section and end with a commentary-specific question. Do not include the source URL in the content panel brief.`;
