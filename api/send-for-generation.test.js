...
import { generationPromptFormula, locateGenerationRow, numericSheetIdentifier, verifyGenerationValues } from "./send-for-generation.js";
...
test("writes valid identifiers as numeric sheet values", () => {
  assert.equal(numericSheetIdentifier("29"), 29);
  assert.equal(numericSheetIdentifier(7), 7);
  assert.equal(numericSheetIdentifier("not-an-id"), "");
});
