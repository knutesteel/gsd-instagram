...
export default async function handler(req, res) {
  ...
  const prompt = `Apply the saved ICP Prompt, Voice Prompt, and Image Prompt to create a ${panelCount || 5}-panel Instagram ${postType || "carousel"}.\n\nArticle title: ${title}\nArticle URL: ${url}\nGSD engagement score: ${score}\n\nArticle-specific setting:\n${setting}\n\nArticle-specific panel content:\n${content}\n\nCaption:\n${caption || ""}\n\nVOICE PROMPT:\n${guide("voice_guide")}\n\nICP PROMPT:\n${guide("icp")}\n\nIMAGE PROMPT:\n${guide("visual_guide")}\n\nThe saved prompts control voice, character identity, style, scale, wardrobe, and visual continuity. The article-specific content above controls only the panel story, actions, reactions, and dialogue. Treat every Panel, Action, Hank, and the squirrel line as mandatory. Every panel must visibly include both characters and their exact two-way exchange. Preserve the stated article-specific setting and carry story props/actions logically across panels; do not reset the scene or replace dialogue with narration, overlays, or a one-character panel.`;
