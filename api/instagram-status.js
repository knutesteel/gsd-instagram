...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [connections, posts, prospects, savedItems] = await Promise.all([
      ...
      fetchRows(
        `${config.supabaseUrl}/rest/v1/instagram_saved_items?user_id=eq.${userId}&select=id,instagram_url,shortcode,media_type,title,content_overview,saved_at,imported_at,review_status&order=saved_at.desc.nullslast,imported_at.desc&limit=10000`,
        config,
      ),
    ]);
